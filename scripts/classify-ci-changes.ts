...
const isNativePath = filePath =>
    ...
    filePath === "scripts/check-native-public-corpus.ts" ||
    filePath === "scripts/engine-abi.ts" ||
    filePath === "scripts/generate-native-property-catalog.ts" ||
    filePath === "scripts/native-crash-worker.ts" ||
    filePath === "scripts/public-crash-worker.ts" ||
...
const isAutomationPath = filePath =>
    ...
    filePath === "scripts/classify-ci-changes.ts" ||
...
const isBrowserPath = filePath =>
    ...
    filePath === "scripts/generate-chromium-properties.ts" ||
    filePath === "scripts/generate-native-grammar-inventory.ts" ||
    filePath.startsWith("scripts/generate-property-value-") ||
    filePath === "scripts/generate-webref-property-branches.ts" ||
    filePath === "scripts/lib/webref-syntax-samples.ts" ||
    filePath === "scripts/check-property-value-matrix.ts" ||
...
const isPackagePath = filePath =>
    filePath.startsWith("src/") ||
    filePath === "scripts/verify-release.ts" ||
