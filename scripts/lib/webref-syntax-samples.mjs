...
const recursiveValues = new Map([
  ["color", "red"],
]);

function joinCss(parts, separator = " ") {
  return parts
    .filter(Boolean)
    .join(separator)
    .replace(/\(\s+/g, "(")
...
function satisfiesRange(value, range) {
  if (!range) return true;
  const numeric = numericPrefix(value);
  if (numeric === null) return true;
  if (Number.isFinite(range.min) && numeric < range.min) return false;
  if (Number.isFinite(range.max) && numeric > range.max) return false;
  return true;
}

function deduplicate(samples) {
  const seen = new Set();
  const output = [];
...
function pairwiseProduct(sampleSets, path, reverse = false) {
...
function repeatedSamples(samples, node, path) {
...
function anyOrderSamples(sampleSets, path) {
  ...
  for (let left = 0; left < nonEmpty.length; left += 1) {
    if (nonEmpty[left].length === 0) continue;
    for (let right = left + 1; right < nonEmpty.length; right += 1) {
      if (nonEmpty[right].length === 0) continue;
      output.push({
        value: joinCss([nonEmpty[left][0].value, nonEmpty[right][0].value]),
        branch: `${path}/subset:${left},${right}`,
      });
      output.push({
        value: joinCss([nonEmpty[right][0].value, nonEmpty[left][0].value]),
...
export function generateWebrefSyntaxSamples({
  ...
  function primitiveSamples(node, path) {
  ...
  function expand(node, path, stack, depth) {
