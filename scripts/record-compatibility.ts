...
const geometricGeneratorBytes = await readFile(
  path.join(repositoryRoot, "scripts/browser-geometric-differential.ts"),
);
...
const processSafetyContractSha256 = createHash("sha256")
  .update(await readFile(path.join(repositoryRoot, "scripts/test-native-crash-safety.ts")))
  .digest("hex");
