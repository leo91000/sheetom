...
import { classifyPaths } from "./classify-ci-changes.ts";
...
test("the generated native property catalog does not trigger browser jobs", () => {
    assert.deepEqual(
        classifyPaths([
            "scripts/generate-native-property-catalog.ts",
...
test("native packaging scripts rebuild the native matrix and package artifact", () => {
    for (const filePath of [
        "scripts/collect-native-artifacts.ts",
...
test("release version synchronization rebuilds native package evidence", () => {
    assert.deepEqual(classifyPaths(["scripts/sync-cargo-version.ts"]), {
        ...none,
        native: true,
        package: true,
    });
...
test("the generated Engine ABI Identity rebuilds native and package artifacts", () => {
    assert.deepEqual(classifyPaths(["engine-abi.json", "scripts/engine-abi.ts"]), {
        ...none,
        native: true,
        package: true,
        wasm: true,
...
test("native and public crash workers run the subprocess safety gate", () => {
    assert.deepEqual(
        classifyPaths(["scripts/native-crash-worker.ts", "scripts/public-crash-worker.ts"]),
...
test("browser-backed grammar generators still run browser validation", () => {
    assert.deepEqual(classifyPaths([
        "scripts/generate-native-grammar-inventory.ts",
        "scripts/generate-property-value-observations.ts",
        "scripts/check-property-value-matrix.ts",
    ]), {
        ...none,
        browser: true,
        docs: true,
...
test("Webref grammar sampling reruns browser evidence without native or package matrices", () => {
    assert.deepEqual(classifyPaths([
        "scripts/generate-webref-property-branches.ts",
        "scripts/lib/webref-syntax-samples.ts",
    ]), {
        ...none,
        browser: true,
        docs: true,
...
test("WASM evidence contracts rerun the backend matrix", () => {
  for (const filename of [
    ...
    "scripts/wasm-backend-evidence.ts",
  ]) {
