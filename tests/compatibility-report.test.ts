...
test("compatibility recording verifies and hashes every native WPT report", async () => {
  ...
  try {
    ...
    await writeFile(nativeCorpusReportPath, JSON.stringify({
      ...
      grammarBranches: { passed: 116, total: 116, positive: 85, negative: 31 },
      propertyBranches: { passed: 10, total: 10, positive: 5, negative: 5 },
      valueCapabilities: { passed: 272, total: 272, positive: 181, negative: 91 },
    ...
    assert.equal(
      ...
      "lightningcss-1.33.0-c6a0c3ce-sheetom.53",
    );
    ...
    assert.deepEqual(
      ...
      { profiles: 24, passed: 116, total: 116 },
    );
    assert.deepEqual(
    ...
    assert.deepEqual(
      ...
      { passed: 272, total: 272, positive: 181, negative: 91 },
    );
    assert.deepEqual(
      {
