...
4. Review the version, changelog, npm and Cargo lockfiles, Native Engine evidence, Compatibility Report, and green release-PR CI. RC6 reports must identify the exact vendored source manifest, 129/129 shorthands, 26 codec profiles, the complete versioned grammar and value-capability corpora, all 66,123 Chromium property/value checks across 711 properties and 93 probes, and complete native/public process-safety execution. The verifier derives the exact corpus counts from reviewed release evidence so this guide cannot silently become a stale numeric allowance.
