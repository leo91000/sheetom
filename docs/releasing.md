...
6. Successful `main` CI builds a binary-free root tarball plus all thirteen exact-version `@sheetom/native-*` tarballs. It executes every native package on its matching operating system, CPU, and Linux libc, including foreign-architecture Node through QEMU, before uploading one immutable artifact set. The Release workflow downloads those tested bytes, rejects an incomplete or misplaced native set, publishes and verifies every platform package before the root package through Trusted Publishing under `next`, and attaches the complete manifest to the draft GitHub prerelease. A manual Release rerun resolves the successful CI run for the unchanged release commit; it never repacks the checkout.
